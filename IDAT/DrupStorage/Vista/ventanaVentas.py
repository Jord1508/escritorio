...
class VentanaVentas(QtWidgets.QMainWindow):
    ...
    def buscarCliente(self):
        ...
        if self.txtCodigoCliente.text() == "":
            ...
            if pos == -1:
                ...
                self.txtNombres.setText(objCliente.getNombresCliente() + " " + objCliente.getApellidoPaternoCliente()  
                                                + " " + objCliente.getApellidoMaternoCliente())
    ...
    def imprimirFactura(self):
        try:
            if self.txtNumeroDocumento.text() == "":
                ...
                QtWidgets.QMessageBox.information(self, "Imprimir Factura", "La factura se ha impreso correctamente...!!!", 
    ...
    def agregar(self):
        try:
            if int(self.txtCantidad.text()) > int(self.txtStock.text()):
                QtWidgets.QMessageBox.information(self, "Venta", "No se puede vender esa cantidad...!!!", 
                                                          QtWidgets.QMessageBox.Ok)
            elif int(self.txtCantidad.text()) <= 0:
                QtWidgets.QMessageBox.information(self, "Venta", "La cantidad ingresada es incorrecta...!!!", 
                                                          QtWidgets.QMessageBox.Ok)
            else:
                lista.append((self.obtenerItem(),
                              self.obtenerCodigo(),
    ...
    def generarVenta(self):
        if self.tblDetalle.rowCount() == 0:
            ...
            objFact = Factura(self.obtenerNumeroDocumento(), self.obtenerFecha(), self.txtIgv.text(), 
                                  self.txtTotalPagar.text(), self.txtCodigoCliente.text())
            ...
            if aFac.buscarFactura(numeroFactura) == -1:
                ...
                aFac.grabar()
                aPro.grabar()
                QtWidgets.QMessageBox.information(self, "Venta", "Se realizó la venta correctamente...!!!", 
                                                          QtWidgets.QMessageBox.Ok)
                lista.clear()
            else:
    ...
    def guardarDetalleVenta(self):
        for i in range(len(lista)):
            objDetVent = DetalleVenta(self.obtenerNumeroDocumento(), lista[i][0], lista[i][1], 
                                          lista[i][3], lista[i][4])
